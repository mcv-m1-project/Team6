...
function windowCandidates=connected_components(binMask,GlobalPath,fileName,windowSet)
            ...
            for class_image = 1:numel(fields)
                if ~isempty(strfind(fields{class_image},'class'))
                    ...
                    %every other class signal
                        ...
                        rect(4) = rect(4)+1;
                        aspectRatio = rect(3)/rect(4);
                        if(check_template_matching(meanIm,fileName,rect,GlobalPath,binMask,windowSet.(fields{class_image}).minCorrelation))
                            Bounding = [round(rect(1))-1 round(rect(2))-1 rect(3) rect(4)];
                            if (Bounding(1) < 1)
                                Bounding(1) = 1; 
                            end
...
function bool = check_template_matching(meanIm,fileName,rect,GlobalPath,binMask,minCorrelation)
    bool = 0;
    imageSegment = imread([GlobalPath fileName '.jpg']);
    imageSegment = imcrop(imageSegment,rect);
    maskSegment = imcrop(binMask,rect);
    imageSegment = rgb2gray(imageSegment);
    imageSegment = single(imageSegment).*single(maskSegment);
    imageSegment = imresize(imageSegment,size(meanIm));
    correlation = corr2(double(imageSegment),meanIm);
