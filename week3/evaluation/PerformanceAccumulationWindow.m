function [TP,FN,FP] = PerformanceAccumulationWindow(detections, annotations)
    ...
    for i=1:size(annotations,1),
        for j=1:size(detections,1),
            if detectionsUsed(j)==0 && annotationsUsed(i) == 0 && RoiOverlapping(annotations(i), detections(j)) > 0.5
                TP = TP+1;
                detectionsUsed(j) = 1;
                annotationsUsed(i) = 1;
            elseif(detectionsUsed(j)==0 && RoiOverlapping(annotations(i), detections(j)) > 0.5)
                detectionsUsed(j) = 1;
            end
        end
    end
